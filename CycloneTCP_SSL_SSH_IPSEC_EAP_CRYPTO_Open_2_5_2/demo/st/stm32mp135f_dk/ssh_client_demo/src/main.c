...
#define APP_IF1_HOST_NAME "ssh-client-demo-1"
...
#define APP_IF2_HOST_NAME "ssh-client-demo-2"
...
error_t shellClientTest(NetInterface *interface)
{
   error_t error;
   size_t n;
   uint32_t exitStatus;
   ...
   do
   {
      ...
      error = getHostByName(interface, APP_SSH_SERVER_NAME, &ipAddr, 0);
      //Any error to report?
      if(error)
      {
         //Debug message
      ...
      //Bind the shell client to the relevant network interface
      error = shellClientBindToInterface(&shellClientContext, interface);
      //Any error to report?
      if(error)
         break;
      ...
      //Debug message
      TRACE_INFO("Command exit status: %" PRIu32 "\r\n", exitStatus);
   ...
   while(1)
   {
      ...
      if(!BSP_PB_GetState(BUTTON_TAMPER))
      {
         if(netGetLinkState(&netInterface[0]))
         {
            ...
            shellClientTest(&netInterface[0]);
         }
         else if(netGetLinkState(&netInterface[1]))
         {
            //Shell client test routine
            shellClientTest(&netInterface[1]);
         }
