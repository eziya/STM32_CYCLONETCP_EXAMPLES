...
#define APP_IF1_HOST_NAME "scp-client-demo-1"
...
#define APP_IF2_HOST_NAME "scp-client-demo-2"
...
error_t scpClientTest(NetInterface *interface)
{
   error_t error;
   size_t n;
   IpAddr ipAddr;
   ...
   do
   {
      ...
      error = getHostByName(interface, APP_SCP_SERVER_NAME, &ipAddr, 0);
      //Any error to report?
      if(error)
      {
         //Debug message
      ...
      //Bind the SCP client to the relevant network interface
      error = scpClientBindToInterface(&scpClientContext, interface);
      //Any error to report?
      if(error)
         break;
   ...
   while(1)
   {
      ...
      if(!BSP_PB_GetState(BUTTON_TAMPER))
      {
         if(netGetLinkState(&netInterface[0]))
         {
            ...
            scpClientTest(&netInterface[0]);
         }
         else if(netGetLinkState(&netInterface[1]))
         {
            //SCP client test routine
            scpClientTest(&netInterface[1]);
         }
