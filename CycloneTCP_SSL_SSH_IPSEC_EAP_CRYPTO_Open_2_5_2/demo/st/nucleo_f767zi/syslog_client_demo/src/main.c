...
#define APP_HOST_NAME "syslog-client-demo"
...
#define APP_SYSLOG_SERVER_NAME "192.168.0.100"
#define APP_SYSLOG_SERVER_PORT 514
...
SyslogClientContext syslogClientContext;
...
void userTask(void *param)
{
   ...
   while(1)
   {
      ...
      {
         //Send Syslog message
         syslogClientSendMessage(&syslogClientContext, SYSLOG_FACILITY_LOCAL0,
            SYSLOG_SEVERITY_INFO, "User button pressed");
         ...
         //Send Syslog message
         syslogClientSendMessage(&syslogClientContext, SYSLOG_FACILITY_LOCAL0,
            SYSLOG_SEVERITY_INFO, "User button released");
      }
...
int_t main(void)
{
   ...
   TRACE_INFO("\r\n");
   TRACE_INFO("*************************************\r\n");
   TRACE_INFO("*** CycloneTCP Syslog Client Demo ***\r\n");
   TRACE_INFO("*************************************\r\n");
   ...
   error = syslogClientInit(&syslogClientContext);
   //Failed to initialize Syslog client?
   if(error)
   {
      //Debug message
      TRACE_ERROR("Failed to initialize Syslog client!\r\n");
   }

   //Specify the IP address of the Syslog server
   ipStringToAddr(APP_SYSLOG_SERVER_NAME, &ipAddr);
   syslogClientConnect(&syslogClientContext, &ipAddr, APP_SYSLOG_SERVER_PORT);
   ...
   NetInterface *interface, DhcpState state)
{
   ...
   if(state == DHCP_STATE_BOUND)
   {
      ...
      {
         //Send Syslog message
         syslogClientFormatMessage(&syslogClientContext,
            SYSLOG_FACILITY_LOCAL0, SYSLOG_SEVERITY_INFO,
            "My IP address is %s", ipv4AddrToString(ipv4Addr,temp));
      }
   }
}
