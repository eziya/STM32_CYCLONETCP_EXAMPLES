...
#define APP_SSH_SERVER_ECDSA_P384_PUBLIC_KEY "keys/server_ecdsa_p384_public_key.pem"
#define APP_SSH_SERVER_ECDSA_P384_PRIVATE_KEY "keys/server_ecdsa_p384_private_key.pem"
...
int_t main(void)
{
   ...
   UTIL_LCD_SetBackColor(LCD_COLOR_BLUE);
   UTIL_LCD_SetTextColor(LCD_COLOR_WHITE);
   UTIL_LCD_SetFont(&Font20);
   BSP_LCD_DisplayOn(0);

   //Clear LCD display
   UTIL_LCD_Clear(LCD_COLOR_BLUE);
...
error_t sshServerImportKeys(SshServerContext *context)
{
   ...
   //Get server's ECDSA P-384 public key
   error = resGetData(APP_SSH_SERVER_ECDSA_P384_PUBLIC_KEY,
      (const uint8_t **) &publicKey, &publicKeyLen);
   //Any error to report?
   if(error)
      return error;

   //Get server's ECDSA P-384 private key
   error = resGetData(APP_SSH_SERVER_ECDSA_P384_PRIVATE_KEY,
      (const uint8_t **) &privateKey, &privateKeyLen);
   //Any error to report?
   if(error)
      return error;

   //Load ECDSA P-384 key pair
