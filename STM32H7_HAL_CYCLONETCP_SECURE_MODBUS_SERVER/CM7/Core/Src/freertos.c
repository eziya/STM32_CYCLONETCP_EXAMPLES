...
error_t modbusServerTlsInitCallback(ModbusClientConnection *connection, TlsContext *tlsContext)
{
  ...
  if(error)
  {
    TRACE_ERROR("tlsSetPrng failed: %d\r\n", error);
    return error;
  }
  ...
  if(error)
  {
    TRACE_ERROR("tlsSetCache failed: %d\r\n", error);
    return error;
  }

  /* Client authentication is ignored */
  error = tlsSetClientAuthMode(tlsContext, TLS_CLIENT_AUTH_NONE);
  if(error)
  {
    TRACE_ERROR("tlsSetClientAuthMode failed: %d\r\n", error);
    return error;
  }

  /* Get server's certificate from resource manager */
  error = resGetData(APP_MODBUS_SERVER_CERT, (const uint8_t **) &serverCert, &serverCertLen);
  if(error)
  {
    TRACE_ERROR("resGetData(APP_MODBUS_SERVER_CERT) failed: %d\r\n", error);
    return error;
  }

  /* Get server's private key from resource manager */
  error = resGetData(APP_MODBUS_SERVER_KEY, (const uint8_t **) &serverKey, &serverKeyLen);
  if(error)
  {
    TRACE_ERROR("resGetData(APP_MODBUS_SERVER_KEY) failed: %d\r\n", error);
    return error;
  }

  /* Get the list of trusted CA certificates from resource manager */
  error = resGetData(APP_MODBUS_CA_CERT, (const uint8_t **) &caList, &caListLen);
  if(error)
  {
    TRACE_ERROR("resGetData(APP_MODBUS_CA_CERT) failed: %d\r\n", error);
    return error;
  }

  /* Load certificate and private key */
  error = tlsLoadCertificate(tlsContext, 0, serverCert, serverCertLen, serverKey, serverKeyLen, NULL);
  if(error)
  {
    TRACE_ERROR("tlsLoadCertificate failed: %d\r\n", error);
    return error;
  }

  /*  Import trusted CA certificates for client verification */
  error = tlsSetTrustedCaList(tlsContext, caList, caListLen);
  if(error)
  {
    TRACE_ERROR("tlsSetTrustedCaList failed: %d\r\n", error);
    return error;
  }
