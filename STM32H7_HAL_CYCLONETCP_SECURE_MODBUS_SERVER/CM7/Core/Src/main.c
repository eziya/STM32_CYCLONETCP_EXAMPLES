...
error_t InitCrypto(void)
{
  ...
  if((err = stm32h7xxCryptoInit()) != NO_ERROR)
  {
    TRACE_ERROR("Failed to initialize hardware crypto accelerator!\r\n");
    return err;
  }

  if((err = trngGetRandomData(seed, sizeof(seed))) != NO_ERROR)
  {
    TRACE_ERROR("Failed to generate random data!\r\n");
    return err;
  }

  if((err = hmacDrbgInit(&hmacDrbgContext, SHA256_HASH_ALGO)) != NO_ERROR)
  {
    TRACE_ERROR("Failed to initialize PRNG!\r\n");
    return err;
  }

  if((err = hmacDrbgSeed(&hmacDrbgContext, seed, sizeof(seed))) != NO_ERROR)
  {
...
/* USER CODE END Boot_Mode_Sequence_2 */
  ...
  TRACE_INFO("\r\n");
  TRACE_INFO("********************************************\r\n");
  TRACE_INFO("*** CycloneTCP Secure Modbus Server Demo ***\r\n");
  TRACE_INFO("********************************************\r\n");
  TRACE_INFO("Target: STM32H747I-DISCO\r\n");
  TRACE_INFO("\r\n");

  /* Initialize RNG & crypto block */
  TRACE_INFO("Starting to initialize crypto block...\r\n");
  if(InitCrypto() != NO_ERROR)
  {
    Error_Handler();
  }

  /* Initialize network stack */
  TRACE_INFO("Starting to Initialize network stack...\r\n");
