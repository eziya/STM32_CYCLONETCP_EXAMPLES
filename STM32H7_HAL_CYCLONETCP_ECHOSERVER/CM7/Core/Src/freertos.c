...
void EchoServerTask(void *argument)
{
  int listen_sock = SOCKET_ERROR, client_sock = SOCKET_ERROR;
  ...
  if(listen_sock < 0)
  {
    TRACE_ERROR("Socket creation failed!\r\n");
    vTaskDelete(NULL);
  }
  ...
  server_addr.sin_port = htons(APP_ECHO_SERVER_PORT);
  server_addr.sin_addr.s_addr = INADDR_ANY;

  if(bind(listen_sock, (struct sockaddr*) &server_addr, sizeof(server_addr)) < 0)
  {
    TRACE_ERROR("Socket bind failed!\r\n");
  ...
  {
    TRACE_ERROR("Socket listen failed!\r\n");
    closesocket(listen_sock);
    vTaskDelete(NULL);
  }

  TRACE_INFO("Echo server (BSD socket) is listening on port %d\r\n", APP_ECHO_SERVER_PORT);
  ...
  while(1)
  {
    ...
    TRACE_INFO("Client connected: %u.%u.%u.%u:%u\r\n", (unsigned int )(client_addr.sin_addr.s_addr & 0xFF),
        (unsigned int )((client_addr.sin_addr.s_addr >> 8) & 0xFF), (unsigned int )((client_addr.sin_addr.s_addr >> 16) & 0xFF),
        (unsigned int )((client_addr.sin_addr.s_addr >> 24) & 0xFF), ntohs(client_addr.sin_port));
    ...
    while(1)
    {
      ...
      if(n <= 0)
      {
        TRACE_INFO("Client disconnected\r\n");
        break;
      }
